...
namespace CarCare.Application.Services
{
    public class AuthManager : IAuthService
    {
        ...
        public async Task<ServiceResponse<TokenDto>> LoginAsync(LoginDto r)
        {
            ...
            if (user == null) return new ServiceResponse<TokenDto>("Kullanýcý adý veya þifre hatalý", false);

            if (user.LockoutEnd.HasValue && user.LockoutEnd.Value > DateTime.UtcNow)
            {
                var timeLeft = Math.Ceiling((user.LockoutEnd.Value - DateTime.UtcNow).TotalMinutes);
                return new ServiceResponse<TokenDto>($"Hesabýnýz kilitlendi. Lütfen {timeLeft} dakika sonra tekrar deneyin.", false);
            }
            ...
            if (user.PasswordHash != r.Password) 
            {
                ...
                if (user.FailedLoginAttempts >= lockoutThreshold)
                {
                    ...
                    return new ServiceResponse<TokenDto>($"Çok fazla baþarýsýz deneme. Hesabýnýz {lockoutMinutes} dakika kilitlendi.", false);
                }

                await _repo.UpdateAsync(user);
                return new ServiceResponse<TokenDto>("Kullanýcý adý veya þifre hatalý", false);
            }
        ...
        public async Task<ServiceResponse<bool>> DeleteUserAsync(int id)
        {
            ...
            if (user == null) return new ServiceResponse<bool>("Kullanýcý bulunamadý", false);
            // Kendi kendini silme kontrolü eklenebilir ama controller'dan kimin çaðýrdýðýný bilmiyoruz (ancak context'ten alabiliriz, þimdilik basit tutalým)
