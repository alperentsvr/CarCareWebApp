...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "Car Care Panel API", Version = "v1" });
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        var context = services.GetRequiredService<CarCareDbContext>();
        // Docker üzerinde veritabanı bağlantısı bazen gecikebilir, basit bir retry mekanizması
        int retries = 3;
        while (retries > 0)
        {
            try
            {
                ...
                await CarCare.Persistence.Seeds.ContextSeed.SeedAsync(context);
